...
export default function Rankings() {
  const [filterScores, setFilterScores] = useState([
    "&page=1",
    "?order[points]=desc&exists[category]=false",
  ]);
  const { currentData, error, isFetching, isSuccess } = useGetScoresQuery({
    page: filterScores[0],
    filter: filterScores[1],
  });
  /*const options = {
        responsive: true,
        plugins: {
    ...
    <div>
      <div>
        <div className="text-center d-flex flex-column">
          <h2>Classements</h2>
          <p>
            Ici est affiché le classement des membres les plus actifs de la
